async function drawChart() {
    ...
    const dataset = await d3.csv(pathToJSON)

    const sexAccessor = d => d.is_retweet
    const sexes = ["female", "male"]
    const sexIds = d3.range(sexes.length)

    const educationAccessor = d => d3.time.format("%H").parse(d.created_at)
    ...
    const sesAccessor = d => d3.time.format("%Y").parse(d.created_at)
    const sesNames = ["2017", "2018", "2019"]
    ...
    dataset.forEach(startingPoint => {
        ...
        stackedProbabilities[key] = timeofDay.map((Hours, i) => {
            stackedProbability += (startingPoint[Hours] / 100)
    ...
    function generatePerson(elapsed) {
        ...
        const education = educationAccessor
